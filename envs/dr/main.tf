...
# ---- Additional Mendix KV secrets (passwords + storage key) ----
resource "random_password" "mendix_db" {
  length  = 24
  special = true
  override_special = "!@#$%^&*()-_=+"
}
...
resource "azurerm_key_vault_secret" "db_password" {
  name         = "MendixDatabasePassword"
  value        = random_password.mendix_db.result
  key_vault_id = module.kv.id
  depends_on   = [azurerm_role_assignment.kv_secrets_officer_runner]
}

# Storage keys are disabled on the SA; placeholder lets the CSI mount succeed.
# Real SA access for Mendix should use the workload MI (Storage Blob/File Data Contributor).
resource "azurerm_key_vault_secret" "storage_account_key" {
  name         = "MendixStorageAccountKey"
  value        = "managed-identity-only-no-shared-key"
  key_vault_id = module.kv.id
  depends_on   = [azurerm_role_assignment.kv_secrets_officer_runner]
}
